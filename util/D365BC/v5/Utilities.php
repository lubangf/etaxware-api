 ...
 * @date: 10-05-2026
 * @file: Utilities.php
 * @path: ./util/D365BC/v5/Utilities.php
...
Class Utilities{
	...
	/**
	 * @name querycommoditycategory
	 * @desc query a a commodity category by issue date
	 * @return JSON-encoded object
	 * @param $userid int, $commoditycategory string
	 *
	 */
	function querycommoditycategory($userid, $commoditycategory){
	    $web = \Web::instance();
	    $url = $this->appsettings['EFRIS_ENDPOINT'];//api endpoint
	    $content = json_encode(new stdClass);// create an empty JSON
	    //date_default_timezone_set('UTC');//set timezone to UTC
	    $this->logger->write("Utilities : querycommoditycategory() : The current time is " . date('Y-m-d H:i:s'), 'r');
	    $this->logger->write("Utilities : querycommoditycategory() : The current timezone is " . date_default_timezone_get(), 'r');
	    ...
	    $this->logger->write("Utilities : querycommoditycategory() : The user id is " . $userid, 'r');
	    $this->logger->write("Utilities : querycommoditycategory() : The CC is " . $commoditycategory, 'r');
	    
	    if (trim($commoditycategory) == '' || empty($commoditycategory)) {
	        return json_encode(array('returnCode' => '999', 'returnMessage' => 'Unknown Error'));
	        $this->logger->write("Utilities : querycommoditycategory() : The Commodity Category is empty.", 'r');
	    }
	    
	    try {
	        $this->logger->write("Utilities : querycommoditycategory() : Querying the Commodity Category started", 'r');
	        $header = array('Content-Type: application/json');
	        $interfaceCode = 'T146';
	        $tcsdetails = new tcsdetails($this->db);
	        $tcsdetails->getByID($this->appsettings['EFRIS_TCS_RECORD_ID']);
	        ...
	        $cc_u = array(
	            'categoryCode' => strval($commoditycategory),
	        ...
	        $cc_u = json_encode($cc_u); //JSON-ifiy
	        $cc_u = base64_encode($cc_u); //base64 encode
	        $this->logger->write("Utilities : querycommoditycategory() : The encoded data is " . $cc_u, 'r');
	        ...
	        $returninfo = $j_response['returnStateInfo'];
	        $content = $j_response['data']['content'];
	        $this->logger->write("Utilities : querycommoditycategory() : The response content is: " . $content, 'r');
	        ...
	        if ($returninfo['returnCode'] == '00') {
	            $this->logger->write("Utilities : querycommoditycategory() : The API call was successful. The return code is: " . $returninfo['returnCode'], 'r');
	            if ($dataDesc['zipCode'] == '1') {
	                $this->logger->write("Utilities : querycommoditycategory() : The response is zipped", 'r');
	                $content = gzdecode(base64_decode($content));
	            } else {
	                $this->logger->write("Utilities : querycommoditycategory() : The response is NOT zipped", 'r');
	                $content = base64_decode($content);
	            }
	        } else {
	            $this->logger->write("Utilities : querycommoditycategory() : The API call was not successful. The return code is: " . $returninfo['returnCode'] . ' - ' . $returninfo['returnMessage'], 'r');
	            $content = json_encode(array('returnCode' => $returninfo['returnCode'], 'returnMessage' => $returninfo['returnMessage']));
	        }
	        
	        return $content;
	    } catch (Exception $e) {
	        $this->logger->write("Utilities : querycommoditycategory() : Error " . $e->getMessage(), 'r');
	        return $content;
	    }
	}
