 ...
 * @date: 09-05-2026
 * @file: Utilities.php
 * @path: ./util/FTS/v15/Utilities.php
...
Class Utilities{
    ...
    function logstockadjustment($userid, $productcode, $batchno, $qty, $suppliertin, $suppliername, $stockintype, $productiondate, $unitprice, $operationtype, $vchtype, $vchtypename, $vchnumber, $vchref, $adjustmenttype, $remarks) {
		// 2026-04-25: normalize ERP inputs to mapped internal codes before persistence.
		$resolvedProductCode = strtoupper(trim($productcode));
		if (trim($resolvedProductCode) != '') {
			$pdct = new products($this->db);
			$pdct->getByErpCode($resolvedProductCode);
			if (!$pdct->dry() && trim($pdct->code) != '') {
				$resolvedProductCode = strtoupper(trim($pdct->code));
			}
		}

		$resolvedStockInType = $stockintype;
		if (trim((string)$stockintype) != '') {
			$mappedStockInType = $this->mapstockincode(trim((string)$stockintype));
			$resolvedStockInType = (trim((string)$mappedStockInType) != '') ? $mappedStockInType : $stockintype;
		}

		$resolvedAdjustmentType = $adjustmenttype;
		if (trim((string)$adjustmenttype) != '') {
			$mappedAdjustmentType = $this->mapstockadjustmentcode(trim((string)$adjustmenttype));
			$resolvedAdjustmentType = (trim((string)$mappedAdjustmentType) != '') ? $mappedAdjustmentType : $adjustmenttype;
		}

        $operationtype = empty($operationtype) || is_null($operationtype)? 'NULL' : $operationtype;
		$adjustmenttype = empty($resolvedAdjustmentType) || is_null($resolvedAdjustmentType)? 'NULL' : $resolvedAdjustmentType;
		$stockintype = empty($resolvedStockInType) || is_null($resolvedStockInType)? 'NULL' : $resolvedStockInType;
                                    ...
                                    . $stockintype . ', "'
                                        . addslashes($batchno) . '", "'
                                            . $productiondate . '", '
                                                . $qty . ', '
                                                    . $unitprice . ', "'
														. addslashes($resolvedProductCode) . '", "'
                                                            . addslashes($vchtype) . '", "'
    ...
    function createdebitnote($debitnotedetails, $goods, $taxes, $buyer, $userid){
        ...
        try{
			...
			// Maintainer note (2026-04-14): v8 backfill for debit-note persistence fields.
			$debitnotedetails['origrossamount'] = empty($debitnotedetails['origrossamount'])? '0' : $debitnotedetails['origrossamount'];
            ...
            $sql = 'INSERT INTO tbldebitnotes
                                    ...
                                    isrefund,
                                    vouchertype,
									vouchertypename, currencyRate, reasoncode, reason,
                                    inserteddt,
                                    insertedby,
                                    modifieddt,
                                    modifiedby)
                                                                                                                                        ...
                                                                                                                                        . $debitnotedetails['isrefund'] . ', "'
                                                                                                                                            . addslashes($debitnotedetails['vchtype']) . '", "'
                                                                                                                                                . addslashes($debitnotedetails['vchtypename']) . '", '
                                                                                                                                                    . $currencyRate . ', "'
																																						. addslashes($debitnotedetails['reasoncode']) . '", "'
																																							. addslashes($debitnotedetails['reason']) . '", "'
                                                                                                                                                    . date('Y-m-d H:i:s') . '", '
                                                                                                                                                        . $userid . ', "'
                                                                                                                                                            . date('Y-m-d H:i:s') . '", '
                                                                                                                                                                . $userid . ')';
                                                                                                                                                                ...
                                                                                                                                                                $this->db->exec(array($sql));
																																								$this->logger->write("Utilities : createdebitnote() : The debit note has been added", 'r');
    ...
    function createcreditnote($creditnotedetails, $goods, $taxes, $buyer, $userid){
        ...
        try{
			...
			// 2026-04-25: persist mapped values where ERP-facing codes are supplied.
			$mappedCurrency = $this->mapcurrency($creditnotedetails['currency']);
			$creditnotedetails['currency'] = (trim((string)$mappedCurrency) != '') ? $mappedCurrency : $creditnotedetails['currency'];

			$mappedIndustryCode = $this->mapindustrycode($creditnotedetails['invoiceindustrycode']);
			$creditnotedetails['invoiceindustrycode'] = (trim((string)$mappedIndustryCode) != '') ? $mappedIndustryCode : $creditnotedetails['invoiceindustrycode'];

			$normalizedReason = $this->normalizecreditnotereason($creditnotedetails['reasoncode'], $creditnotedetails['reason']);
			$creditnotedetails['reasoncode'] = $normalizedReason['reasoncode'];
			$creditnotedetails['reason'] = $normalizedReason['reason'];
    ...
    /**
     * @name mapexciseduty
     * @desc return the standard code for an excise duty
     * @return string
     * @param $no string
     *
     */
    function mapexciseduty($code){
        /**
         * 1. Cleanup the units
	...
	/**
	 * @name normalizecreditnotereason
	 * @desc Normalize credit/debit reason code only. Reason text remains free text from ERP.
	 * @return array
	 * @param $reasoncode string, $reason string
	 */
	function normalizecreditnotereason($reasoncode, $reason){
		$rawCode = trim((string)$reasoncode);
		$rawReason = trim((string)$reason);

		$normalizedCode = trim((string)$this->mapreasoncode($rawCode));
		if ($normalizedCode == '') {
			$normalizedCode = $rawCode;
		}

		$this->logger->write('Utilities : normalizecreditnotereason() : normalized code=' . $normalizedCode . ', reason remains ERP free text', 'r');
    ...
    /**
     * @name computeExciseDuty
     * @desc compute excise duty
     * @return array
     * @param $userid string, $product_code string, $qty float, $unit float, $amount float, $discount float, $discountpct float
     *
     */
    function computeExciseDuty($userid, $product_code, $qty, $unit, $amount, $discount, $discountpct){
        $tax_amount = 0;
    ...
    function updateproduct($product, $userid){
        ...
        try{
            ...
            $sql = 'UPDATE tblproductdetails SET 
                                    ...
                                    haveCustomsUnit = ' . $product['haveCustomsUnit'] . ',
                                    pack = ' . $product['pack'] . ',
                                    stick = ' . $product['stick'] . ',
                                    exciseDutyName = "' . addslashes($product['exciseDutyName']) . '",
                                    exciseRate = "' . addslashes($product['exciseRate']) . '",
                                    modifieddt = "' .  date('Y-m-d H:i:s') . '", 
                                    modifiedby = ' . $userid  . ' 
	...
	function stockin($userid, $branchuraid, $productcode, $batchno, $qty, $suppliertin, $suppliername, $stockintype, $productiondate, $unitprice){
	    ...
	    try {
	        ...
	        $resolvedStockInType = $this->mapstockincode($stockintype);
	        $resolvedStockInType = (trim((string)$resolvedStockInType) != '') ? $resolvedStockInType : $stockintype;

	        // 2026-04-25: v12 stock-in pricing hardening - never default unit price to 1/blank.
	        $resolvedUnitPrice = trim($unitprice);
	        if ($resolvedUnitPrice == '' || $resolvedUnitPrice == '0' || $resolvedUnitPrice == '0.00') {
	            if (trim($product->purchaseprice) != '' && trim($product->purchaseprice) != '0' && trim($product->purchaseprice) != '0.00') {
	                $resolvedUnitPrice = trim($product->purchaseprice);
	            } else if (trim($product->unitprice) != '' && trim($product->unitprice) != '0' && trim($product->unitprice) != '0.00') {
	                $resolvedUnitPrice = trim($product->unitprice);
	            } else {
	                $this->logger->write("Utilities : stockin() : The product " . $productcode . " has no valid unit price (request/purchaseprice/unitprice)", 'r');
	                return json_encode(array('returnCode' => '659', 'returnMessage' => 'Unit price is missing for product code ' . $productcode));
	            }
	        }
	        ...
	        $stock = array(
	            'goodsStockIn' => array(
	                ...
	                'stockInType' => $resolvedStockInType,
	            ...
	            'goodsStockInItem' => array(
	                array(
	                    ...
	                    'quantity' => strval($qty),
	                    'unitPrice' => strval($resolvedUnitPrice),
	...
	function batchstockin($userid, $branchuraid, $products, $batchno, $suppliertin, $suppliername, $stockintype, $productiondate){
	    ...
	    try {
	        ...
	        foreach ($products as $obj) {
	            ...
	            if ($product->dry()) {
	                $this->logger->write("Utilities : batchstockout() : The product " . trim($obj['productCode']) . " does not exist", 'r');
	                return json_encode(array('returnCode' => '658', 'returnMessage' => 'goodsCode does not exist!'));
	            }

	            // 2026-04-25: v12 stock-out pricing hardening - derive line unit price without defaulting to 1.
	            $lineUnitPrice = trim($obj['unitPrice']);
	            if ($lineUnitPrice == '' || $lineUnitPrice == '0' || $lineUnitPrice == '0.00') {
	                if (trim($product->purchaseprice) != '' && trim($product->purchaseprice) != '0' && trim($product->purchaseprice) != '0.00') {
	                    $lineUnitPrice = trim($product->purchaseprice);
	                } else if (trim($product->unitprice) != '' && trim($product->unitprice) != '0' && trim($product->unitprice) != '0.00') {
	                    $lineUnitPrice = trim($product->unitprice);
	                } else {
	                    $this->logger->write("Utilities : batchstockout() : The product " . trim($obj['productCode']) . " has no valid unit price (purchaseprice/unitprice)", 'r');
	                    return json_encode(array('returnCode' => '659', 'returnMessage' => 'Unit price is missing for product code ' . trim($obj['productCode'])));
	                }
	            }
	...
	function stockout($userid, $branchuraid, $productcode, $batchno, $qty, $adjustmenttype, $remarks){
	    ...
	    try {
	        ...
	        $resolvedAdjustmentType = $this->mapstockadjustmentcode($adjustmenttype);
	        $resolvedAdjustmentType = (trim((string)$resolvedAdjustmentType) != '') ? $resolvedAdjustmentType : $adjustmenttype;

	        // 2026-04-25: v12 stock-out pricing hardening - never default unit price to 1.
	        $resolvedUnitPrice = '';
	        if (trim($product->purchaseprice) != '' && trim($product->purchaseprice) != '0' && trim($product->purchaseprice) != '0.00') {
	            $resolvedUnitPrice = trim($product->purchaseprice);
	        } else if (trim($product->unitprice) != '' && trim($product->unitprice) != '0' && trim($product->unitprice) != '0.00') {
	            ...
	            $this->logger->write("Utilities : stockout() : The product " . $productcode . " has no valid unit price (purchaseprice/unitprice)", 'r');
	            return json_encode(array('returnCode' => '659', 'returnMessage' => 'Unit price is missing for product code ' . $productcode));
	        }
	        ...
	        $stock = array(
	            'goodsStockIn' => array(
	                ...
	                'supplierTin' => '',
	                'supplierName' => '',
	                'adjustType' => $resolvedAdjustmentType,
	                'remarks' => $remarks,
	                'stockInDate' => '',
	                /**
	                 *Date: 2020-11-18
	...
	function uploadinvoice($userid, $branchuraid, $buyer, $invoicedetails, $goods, $payments, $taxes){
	    ...
	    try {
	        ...
	        try{
	            ...
	            $i = 0;
	            foreach ($goods as $obj) {
	                if (empty($obj['discountflag'])) {
	                    if (isset($obj['discountFlag']) && trim((string)$obj['discountFlag']) !== '') {
	                        $obj['discountflag'] = trim((string)$obj['discountFlag']);
	                    } elseif (isset($obj['DISCOUNTFLAG']) && trim((string)$obj['DISCOUNTFLAG']) !== '') {
	                        $obj['discountflag'] = trim((string)$obj['DISCOUNTFLAG']);
	                    }
	                }

	                if (!empty($obj['discountflag'])) {
	                    $rawDiscountFlag = strtoupper(trim((string)$obj['discountflag']));
	                    if (in_array($rawDiscountFlag, array('YES', 'Y', 'TRUE', '1'), true)) {
	                        $obj['discountflag'] = '1';
	                    } elseif (in_array($rawDiscountFlag, array('NO', 'N', 'FALSE', '0', '2'), true)) {
	                        $obj['discountflag'] = '2';
	                    }
	                }
	    ...
	    try {
	        ...
	        $invoice_u = array(
	            'sellerDetails' => array(
	                ...
	                'emailAddress' => empty($org->emailaddress)? '' : addslashes($org->emailaddress),
	                'placeOfBusiness' => empty($org->placeofbusiness)? '' : addslashes($org->placeofbusiness),
	                'referenceNo' => empty($debitnotedetails['erpdebitnoteid'])? '' : $debitnotedetails['erpdebitnoteid'],
	                'branchId' => empty($branchuraid)? $devicedetails->branchId : $branchuraid,
	                'isCheckReferenceNo' => '0'
	            ),
	            'basicInformation' => array(
	...
	function uploadcreditnote($userid, $buyer, $creditnotedetails, $goods, $payments, $taxes){
	    ...
	    try {
	        ...
	        // Normalize reason fields so API payload and persisted state use platform values.
	        $normalizedReason = $this->normalizecreditnotereason($creditnotedetails['reasoncode'], $creditnotedetails['reason']);
	        $creditnotedetails['reasoncode'] = $normalizedReason['reasoncode'];
	        $creditnotedetails['reason'] = $normalizedReason['reason'];
	...
	function logstocktransfer($userid, $productcode, $qty, $vchtype, $vchtypename, $vchnumber, $vchref, $remarks, $sourceBranchId, $destinationBranchId, $transferTypeCode, $commodityGoodsId) {
	    // 2026-04-25: map ERP-facing transfer values before persistence.
	    $resolvedSourceBranchId = $this->mapbranchcode($sourceBranchId);
	    $sourceBranchId = (trim((string)$resolvedSourceBranchId) != '') ? $resolvedSourceBranchId : $sourceBranchId;

	    $resolvedDestinationBranchId = $this->mapbranchcode($destinationBranchId);
	    $destinationBranchId = (trim((string)$resolvedDestinationBranchId) != '') ? $resolvedDestinationBranchId : $destinationBranchId;
	    ...
	    if ((trim((string)$commodityGoodsId) == '' || is_null($commodityGoodsId)) && !$pdct->dry() && trim($pdct->uraproductidentifier) != '') {
