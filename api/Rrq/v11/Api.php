...
Class Api{
    ...
    protected $reqestBody; //Used to store contents sent in the request body
    ...
    function sendmail(){
        ...
        $this->logger->write("Api : sendmail() : The previous URL is " . $this->f3->get('SERVER.HTTP_REFERER'), 'r');
        $this->logger->write("Api : sendmail() : The current URL is " . $this->f3->get('SERVER.REQUEST_URI'), 'r');
    ...
    function index(){              
        ...
        if ($this->errorcode) {
            ...
            if (isset($json['OPERATIONTYPE'])) {
                $operationType = strtoupper(trim($json['OPERATIONTYPE']));
                
                $this->logger->write("Api : index() : The operation type is: " . $operationType, 'r');
                
                if ($operationType == 'TESTAPI') {
                    self::testapi();
                } elseif($operationType == 'UPLOADINVOICE'){
                    $this->logger->write("Api : index() : Calling the uploadinvoice operation!", 'r');
                    self::uploadinvoice();
                    $this->logger->write("Api : index() : End of the uploadinvoice operation!", 'r');
                } elseif($operationType == 'UPLOADCREDITNOTE'){
                    $this->logger->write("Api : index() : Calling the uploadcreditnote operation!", 'r');
                    self::uploadcreditnote();
                    $this->logger->write("Api : index() : End of the uploadcreditnote operation!", 'r');
                } else {
                    $this->logger->write("Api : index() : The operation type is unknown!", 'r');
                    $this->message = 'The operation type is unknown!';
                    $this->code = '8011';
                ...
                $this->logger->write("Api : index() : The operation type was not specified!", 'r');
                $this->message = 'The operation type was not specified!';
                $this->code = '8010';
        ...
        $this->logger->write("Api : index() : Preparing to exit the function!", 'r');
        $len = sizeof($this->response);
        header ("CONTENT-LENGTH:".$len);
        //print $this->response;
        die(json_encode($this->response));
                    ...
                    if(isset($n_data['records'])){
                        ...
                        if ($n_data['records']) {
                            ...
                            if ($pdct->dry()) {
                                ...
                                $product['description'] = "This product was created by the SAP api";
                                ...
                                $product['description'] = "This product was updated by the SAP api";
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    ...
                    if (isset($json['INVENTORIES'])) {
                        $this->logger->write("Api : uploadinvoice() : INVENTORIES: " . json_encode($json['INVENTORIES']), 'r');
                        foreach ($json['INVENTORIES'] as $obj){
                            ...
                            $this->logger->write("Api : uploadinvoice() : Here 3", 'r');
                            /**
                             * Determine the tax code relevant to the product. SAP will always send a NULL TAXCODE
                             */
                            ...
                            $this->logger->write("Api : uploadinvoice() : Here 4", 'r');
                            if (trim($taxcode) == '' || empty($taxcode)) {
                                ...
                                if (trim($taxid) == '' || empty($taxid)) {
                                    ...
                                    $this->logger->write("Api : uploadinvoice() : Here 6", 'r');
                                    $goods[] = array(
                                        'groupid' => NULL,
                                        'item' => $product->name,
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    ...
                    if(trim($orivchnumber) !== '' || ! empty(trim($orivchnumber))) {
                        ...
                        if ($orig_inv->dry()) {
                            $this->logger->write("Api : uploadcreditnote() : The associated original invoice does not exist in the database", 'r');
                            $oriinvoiceid = NULL;
                            $oriinvoiceno = NULL;
                            
                            $this->message = "The associated original invoice does not exist OR is not yet uploaded!";
                            $this->code = '-999';
                        ...
                        foreach ($json['INVENTORIES'] as $obj){
                            ...
                            if (trim($taxcode) == '' || empty($taxcode)) {
                                ...
                                if (trim($taxid) == '' || empty($taxid)) {
									...
									//Date: 2025-04-08
									//Author: Francis Lubanga
									//Description: Handle the rate unit from SAP.
									$this->logger->write("Api : uploadcreditnote() : The unit before is " . $unit, 'r');
									$unit = (float)$unit * 100;
									$this->logger->write("Api : uploadcreditnote() : The unit after is " . $unit, 'r');
     ...
     * @desc PHP has no base64UrlEncode function, so let's define one that does some magic by replacing + with -, / with _ and = with ''. This way we can pass the string within URLs without any URL encoding.
     * @param string $text
     * @return mixed
     */
    ...
    function beforeroute(){
        ...
        $this->logger->write("Api : beforeroute() : REMOTE_ADDR = " . $this->f3->get('SERVER.REMOTE_ADDR'), 'r');
        $this->logger->write("Api : beforeroute() : REMOTE_HOST = " . $this->f3->get('SERVER.REMOTE_HOST'), 'r');
        $this->logger->write("Api : beforeroute() : REMOTE_PORT = " . $this->f3->get('SERVER.REMOTE_PORT'), 'r');
        $this->logger->write("Api : beforeroute() : REMOTE_USER = " . $this->f3->get('SERVER.REMOTE_USER'), 'r');
        $this->logger->write("Api : beforeroute() : REDIRECT_REMOTE_USER = " . $this->f3->get('SERVER.REDIRECT_REMOTE_USER'), 'r');
        $this->logger->write("Api : beforeroute() : HTTP_X_FORWARDED_FOR = " . $this->f3->get('SERVER.HTTP_X_FORWARDED_FOR'), 'r');
        
        $headers = getallheaders();
        //print_r($headers);
        $this->logger->write("Api : beforeroute() : The header is: " . json_encode($headers), 'r');
        $this->logger->write("Api : beforeroute() : The raw token is: " . $headers['Authorization'], 'r');
		//Troubleshooting 
		$this->logger->write("Api : beforeroute() : The raw token is: " . $headers['authorization'], 'r');
        ...
        if (isset($headers['Authorization']) || isset($headers['authorization'])) {
			...
			if(isset($headers['Authorization'])) {
				...
				$this->logger->write("Api : beforeroute() : The token has an issue!", 'r');
				
				$this->message = 'The token has an issue!';
                $this->code = '8564';
                    
                $this->errorcode = '8564';
                $this->errormessage = 'The token has an issue!';
                    
                return;
			}
			
            $this->accessToken = $token;
            $this->logger->write("Api : beforeroute() : The token[0] is: " . $token[0], 'r');
            $this->logger->write("Api : beforeroute() : The token[1] is: " . $token[1], 'r');
            $tokenParts = explode('.', $token[1]);
            $header = base64_decode($tokenParts[0]);
            $payload = base64_decode($tokenParts[1]);
            $signatureProvided = $tokenParts[2];
            
            $this->logger->write("Api : beforeroute() : The token header is: " . $header, 'r');
            $this->logger->write("Api : beforeroute() : The token payload is: " . $payload, 'r');
            $this->logger->write("Api : beforeroute() : The token signature is: " . $signatureProvided, 'r');
            
            /*
            {
                "sub": "7bsrm3gc8svfclvcck62eu0mae",
                "token_use": "access",
                "scope": "etaxware-api/testapi etaxware-api/uploadinvoice",
                "auth_time": 1665570032,
                "iss": "https://cognito-idp.us-east-2.amazonaws.com/us-east-2_KxMkRiewo",
                "exp": 1665573632,
                "iat": 1665570032,
                "version": 2,
                "jti": "4f4daf15-4e06-4109-9f4d-8da4fcd770de",
                "client_id": "7bsrm3gc8svfclvcck62eu0mae"
            }*/
            ...
            if (isset(json_decode($payload)->exp)) {
                ...
                $this->logger->write("Api : beforeroute() : expiration: " . $expiration, 'r');
                
                if ($tokenExpired) {
                    $this->logger->write("Api : beforeroute() : The token expired.", 'r');
                    $this->message = 'The token expired.';
                    $this->code = '8004';
                    
                    $this->errorcode = '8004';
                    $this->errormessage = 'The token expired.';
                    
                    return;
                } else {
                    /*
                     //Signature verification part
                     $secret = '1mh6rajfp66kq0ct7pkotn8ivh09hk1q5nj2lr7sm8vbd612ae7v';
                     $clientId = '7bsrm3gc8svfclvcck62eu0mae';
                     ...
                     $this->logger->write("Api : beforeroute() : base64UrlSignature: " . $base64UrlSignature, 'r');
                     $this->logger->write("Api : beforeroute() : signatureProvided: " . $signatureProvided, 'r');
                     $this->logger->write("Api : beforeroute() : signatureProvided: " . $signatureValid, 'r');
                     
                     //Verify the scopes
                     //Verify cient ID
                     */
                    
                    $reqestBody_c = NULL; //Hold the transformed request body
                    
                    if ($this->f3->get('SERVER.REQUEST_METHOD') == 'GET') {
                        $this->logger->write("Api : beforeroute() : GET call", 'r');
                        ...
                        $this->logger->write("Api : beforeroute() : The apikey sent within the URL is: " . $API_KEY, 'r');
                    } elseif ($this->f3->get('SERVER.REQUEST_METHOD') == 'POST'){
                        $this->logger->write("Api : beforeroute() : POST call", 'r');
                    } else {
                        $this->logger->write("Api : beforeroute() : The call type is unsupported", 'r');
                    }
                    //Pick the request body content from the client
                    $this->reqestBody = file_get_contents('php://input');
                    $this->logger->write("Api : beforeroute() : Raw body content is: " . $this->reqestBody, 'r');
                    
                    if (strtoupper($this->appsettings['API_TYPE']) == 'JSON') {
                        $this->logger->write("Api : beforeroute() : The expected format is JSON", 'r');
                        $this->json = $this->reqestBody;
                        $this->logger->write("Api : beforeroute() : The JSON content is: " . $this->json, 'r');
                        $reqestBody_c = json_decode($this->json, TRUE); //convert JSON into array
                        $this->logger->write("Api : beforeroute() : Converted content is: " . json_encode($reqestBody_c), 'r');
                    } elseif (strtoupper($this->appsettings['API_TYPE']) == 'XML'){
                        $this->logger->write("Api : beforeroute() : The expected format is XML", 'r');
                        $this->xml = $this->reqestBody;
                        $this->logger->write("Api : beforeroute() : The xml after replacing special xters" . $this->xml, 'r');
                        $reqestBody_c = htmlspecialchars_decode($this->xml, ENT_XML1);
                        $this->logger->write("Api : beforeroute() : Converted content is: " . json_encode($reqestBody_c), 'r');
                    } else {
                        $this->logger->write("Api : beforeroute() : There is no expected format defined", 'r');
                        $this->message = 'No expected format defined. Please contact the system administrator!';
                        $this->code = '8000';
                        
                        $this->errorcode = '8000';
                        $this->errormessage = 'No expected format defined. Please contact the system administrator!';
                        
                        return;
                    }
                    
                    if (!isset($reqestBody_c)) {
                        $this->logger->write("Api : beforeroute() : reqestBody_c is not set", 'r');
                        ...
                        if (strtoupper($this->appsettings['API_TYPE']) == 'JSON') {
                            if(sizeof($reqestBody_c) == 0 && empty($API_KEY)){
                                
                                $this->logger->write("Api : beforeroute() : reqestBody_c OR API_KEY is not set", 'r');
                                ...
                                $this->logger->write("Api : beforeroute() : The apikey is: " . $reqestBody_c['APIKEY'], 'r');
                                
                                if ($reqestBody_c['APIKEY']) {
                                    $this->apikey = trim($reqestBody_c['APIKEY']);
                                } else {
                                    $this->apikey = $API_KEY;
                                }
                                ...
                                if(empty($this->apikey)){
                                    ...
                                    if($apikey_check->dry ()){
                                        ...
                                        $this->version = $reqestBody_c['VERSION'];
                                        ...
                                        $erpuser = trim($reqestBody_c['ERPUSER']);
                ...
                $this->logger->write("Api : beforeroute() : The expiry time is not set.", 'r');
                
                $this->logger->write("Api : beforeroute() : The expiry time is not set in the token.", 'r');
                $this->message = 'The expiry time is not set in the token.';
                $this->code = '8003';
                
                $this->errorcode = '8003';
                $this->errormessage = 'The expiry time is not set in the token.';
                
                return;
            }           
        } else {
            $this->logger->write("Api : beforeroute() : The token was not supplied.", 'r');
            $this->message = 'The token was not supplied.';
            $this->code = '8001';
            
            $this->errorcode = '8001';
            $this->errormessage = 'The token was not supplied.';
            return;
        }      
    }
