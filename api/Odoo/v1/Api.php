...
Class Api{
    ...
    protected $ccrecipientname = 'Glu.ERP Developer';
    ...
    function sendmail(){
        ...
        if ($recipientemail && $body) {
            try {
                ...
                $mail->setFrom($this->appsettings['EMAILUSERNAME'], 'Glu.ERP App');
                ...
                if ($this->userpermissions[$permission]) {
                    ...
                    if(isset($n_data['records'])){
                        ...
                        if ($n_data['records']) {
                            ...
                            //Add/update the product to gERP
                            ...
                            if ($pdct->dry()) {
                                $this->logger->write("Api : fetchproduct() : The product " . $product['code'] . " does not exist on gERP", 'r');
                                
                                $product['description'] = "This product was created by the TALLY api";
                                $pdct_status = $this->util->createproduct($product, $this->userid_u);
                                
                            } else {
                                $this->logger->write("Api : fetchproduct() : The product " . $product['code'] . " exists on gERP", 'r');
                            ...
                            if ($pdct_status) {
                                $this->logger->write("Api : fetchproduct() : The product " . $product['code'] . " was created/updated on gERP successfully", 'r');
                            } else {
                                $this->logger->write("Api : fetchproduct() : The product " . $product['code'] . " was NOT created/updated on gERP", 'r');
                            }
    ...
    function uploadproduct(){
        ...
        if ($this->errorcode) {
             ...
             * 5. If it is a success, populate gERP product tables
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if (empty($data)) {
                    ...
                    if(isset($n_data['records'])){
                        ...
                        if ($pdct->dry()) {
                            $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " does not exist on gERP", 'r');
                            
                            $product['description'] = "This product was created by the TALLY api";
                            $pdct_status = $this->util->createproduct($product, $this->userid_u);
                            
                        } else {
                            $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " exists on gERP", 'r');
                        ...
                        if ($pdct_status) {
                            $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " was created/updated on gERP successfully", 'r');
                        } else {
                            $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " was NOT created/updated on gERP", 'r');
                        }
                    } else {
                    ...
                    foreach($data as $elem){
                        if(isset($elem['returnCode'])){
                            ...
                            /**
                             * Handle response code 602: Goods Code already exists && the product is not on the gERP database
                             * 1. Check if the product is on gERP
                             * 2. If the product is on gERP, then ignore
                             * 3. If the product is NOT on gERP, call the create product API
                             */
                            ...
                            if (trim($elem['returnCode']) == '602') {
                                ...
                                if ($pdct->dry()) {
                                    ...
                                    if ($pdct_status) {
                                        $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " was created on gERP successfully", 'r');
                                    } else {
                                        $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " was NOT created on gERP", 'r');
                                    }
                                } else {
                                    ;
                                }
    ...
    function uploadinvoice(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    $this->logger->write("Api : uploadinvoice() : The invoice does not exist on gERP. Proceed and upload", 'r');
                    ...
                    if (isset($data['returnCode'])){
                        ...
                        if ($inv_status) {
                            $this->logger->write("Api : uploadinvoice() : The invoice was created on gERP successfully", 'r');
                        } else {
                            $this->logger->write("Api : uploadinvoice() : The invoice was NOT created on gERP", 'r');
                        }
    ...
    function uploadcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    $this->logger->write("Api : uploadcreditnote() : The creditnote does not exist on gERP. Proceed and upload", 'r');
                    ...
                    if(trim($vchref) !== '' || ! empty(trim($vchref))) {
                        ...
                        if ($orig_inv->dry()) {
                             ...
                             * 4. Update the gERP record of this invoice
                             */
                    ...
                    if (isset($data['returnCode'])){
                        ...
                        if ($inv_status) {
                            $this->logger->write("Api : uploadcreditnote() : The credit note was created on gERP successfully", 'r');
                        } else {
                            $this->logger->write("Api : uploadcreditnote() : The credit note was NOT created on gERP", 'r');
                        }
    ...
    function uploaddebitnote(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    $this->logger->write("Api : uploaddebitnote() : The debitnote does not exist on gERP. Proceed and upload", 'r');
                    ...
                    if(trim($vchref) !== '' || ! empty(trim($vchref))) {
                        ...
                        if ($orig_inv->dry()) {
                             ...
                             * 2. If oriinvoiceid is NOT empty, then ignore
                             * 3. If it is empty, then query EFRIS and retrieve it
                             * 4. Update the gERP record for this invoice
                             */
                    ...
                    if (isset($data['returnCode'])){
                        ...
                        if ($inv_status) {
                            $this->logger->write("Api : uploadproduct() : The debit note was created on gERP successfully", 'r');
                        } else {
                            $this->logger->write("Api : uploadproduct() : The debit note was NOT created on gERP", 'r');
                        }
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if ($vchtype == 'Credit Note') {
                    ...
                    if($inv_check->dry ()){
                        $this->logger->write("Api : queryinvoice() : The credit note does not exist on gERP", 'r');
                    ...
                    if($inv_check->dry ()){
                        $this->logger->write("Api : queryinvoice() : The debit note does not exist on gERP", 'r');
                    ...
                    if($inv_check->dry ()){
                        $this->logger->write("Api : queryinvoice() : The invoice does not exist on gERP", 'r');
    ...
    function voidcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    $this->logger->write("Api : voidcreditnote() : The credit note does not exist on gERP", 'r');
                    $this->message = 'The credit note does not exist on EFRIS';
                    $this->code = '99';
                } else {
