...
Class Api{
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                if (isset($data['returnCode'])){
                    ...
                    if ($inv_status) {
                        $this->logger->write("Api : uploadproduct() : The invoice was created on eTW successfully", 'r');
                    } else {
                        $this->logger->write("Api : uploadproduct() : The invoice was NOT created on eTW", 'r');
                    }
    ...
    function uploadcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                if ($orig_inv->dry()) {
                    $this->logger->write("Api : uploadcreditnote() : There is no associated original invoice", 'r');
                    $oriinvoiceid = NULL;
                    $oriinvoiceno = NULL;
                } else {
    ...
    function uploaddebitnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                if ($orig_inv->dry()) {
                    $this->logger->write("Api : uploaddebitnote() : There is no associated original invoice", 'r');
                    $oriinvoiceid = NULL;
                    $oriinvoiceno = NULL;
                } else {
                    $oriinvoiceid = $orig_inv->einvoiceid;
    ...
    function beforeroute(){
        ...
        /*} elseif (!empty($API_KEY)) {
            ...
            if ($xml->APIKEY) {
                $apikey = trim($xml->APIKEY);
            } else {
                $apikey = $API_KEY;
            }

            $this->logger->write("Api : beforeroute() : The new apikey is: " . $apikey, 'r');
            ...
            if(empty($apikey)){
                ...
                $apikey_check->load(array('apikey=? AND status=? AND expirydt > NOW()', $apikey, $this->appsettings['APIKEYENABLEDSTATUS']));
                $this->logger->write($this->db->log(TRUE), 'r');
                ...
                if($apikey_check->dry ()){
                    ...
                    $clientversion = $xml->VERSION;
                    
                    if (trim($clientversion) == $this->appsettings['APPVERSION']) {
                        $this->logger->write("Api : beforeroute() : The client version " . trim($clientversion) . " and api version " . $this->appsettings['APPVERSION'] . " match", 'r');
                    } else {
                    ...
                    //update lastaccessdt
                    try {
                        $this->db->exec(array("UPDATE tblapikeys SET lastaccessdt = '" . date('Y-m-d H:i:s') . "' WHERE apikey = '" . $apikey . "'"));
                    } catch (Exception $e) {
