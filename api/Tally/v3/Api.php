...
Class Api{
    ...
    function validatetin(){
        ...
        if ($this->errorcode) {
            ...
            $user_u->getByErpUserCode(strtoupper($erpuser));
            //$user_u->id
                                 
            $data = $this->util->querytaxpayer($user_u->id, $tin);//will return JSON.
            //var_dump($data);
            ...
            $this->util->createerpauditlog($this->userid, $activity, $windowsuser, $ipaddress, $macaddress, $systemname, $xml);
        }
    ...
    function checktaxpayer(){
        $operation = NULL; //tblevents
        $permission = 'CHECKTAXPAYER'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->checktaxpayer($user_u->id, $tin, $commodity);//will return JSON.
            //var_dump($data);
    ...
    function currencyquery(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->fetchcurrencyrates($user_u->id); // will return JSON.
            // var_dump($data);
            $data = json_decode($data, true);
    ...
    function stockin(){
        ...
        if ($this->errorcode) {
             ...
             * Description: Retrieve the actual userid & branchid based on the ERP-eTW mapping. This is meant to make the eTW plugin multi-branch.
             *              The following operations will attempt to retrive and send the user's branh, the fall back branch will be the HQ branch stored in the table tbltcsdetails
             *              1. stockin, 2. batch stockin, 3. stockout, 4. invoice upload, 5. debit note upload
             */
            ...
            $erpuser = trim($xml->ERPUSER);
            $user_u->getByErpUserCode(strtoupper($erpuser));
            $branch->getByID($user_u->branch);
            
            //$branch->uraid
            //$user_u->id
            
            $data = $this->util->stockin($user_u->id, $branch->uraid, $productcode, $batchno, $qty, $suppliertin, $suppliername, $stockintype, $productiondate, $unitprice);//will return JSON.
            ...
            if(isset($data['returnCode'])){
                ...
                if ($data) {
                    ...
                    $this->code = '00';
                    $this->util->logstockadjustment($this->userid, $productcode, $batchno, $qty, $suppliertin, $suppliername, $stockintype, $productiondate, $unitprice, trim($this->appsettings['STOCKINOPERATIONTYPE']), NULL, NULL, NULL, NULL, NULL, NULL);
                }
    ...
    function batchstockin(){
        ...
        if ($this->errorcode) {
            ...
            if($vch_check->dry ()){
                ...
                $data = $this->util->batchstockin($user_u->id, $branch->uraid, $products, $batchno, $suppliertin, $suppliername, $stockintype, $productiondate);//will return JSON.
                ...
                if(isset($data['returnCode'])){
                    ...
                    if ($data) {
                        ...
                        $this->message = "The operation was successful";
                        $this->code = '00';
                        $this->util->logstockadjustment($this->userid, NULL, $batchno, NULL, $suppliertin, $suppliername, $stockintype, $productiondate, NULL, trim($this->appsettings['STOCKINOPERATIONTYPE']), $vchtype, $vchtypename, $vchnumber, $vchref, NULL, NULL);
                    }
            ...
            $activity = 'BATCHSTOKIN: ' . $vchnumber . ': ' . $vchref . ': ' . $this->message;
            $windowsuser = trim($xml->WINDOWSUSER);
            $ipaddress = trim($xml->IPADDRESS);
            $macaddress = trim($xml->MACADDRESS);
            $systemname = trim($xml->SYSTEMNAME);
    ...
    function stockout(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->stockout($user_u->id, $branch->uraid, $productcode, $batchno, $qty, $adjustmenttype, $remarks);
            
            $data = json_decode($data, true);
            
            if(isset($data['returnCode'])){
                $this->logger->write("Api : stockout() : The operation to increase stock not successful. The error message is " . $data['returnMessage'], 'r');
                $this->message = $data['returnMessage'];
                $this->code = $data['returnCode'];
                ...
                if ($data) {
                    ...
                    $this->message = "The operation was successful";
                    $this->code = '00';
                    $this->util->logstockadjustment($this->userid, $productcode, $batchno, $qty, NULL, NULL, NULL, NULL, NULL, trim($this->appsettings['STOCKOUTOPERATIONTYPE']), NULL, NULL, NULL, NULL, $adjustmenttype, $remarks);
                }
    ...
    function fetchproduct(){
        ...
        if ($this->errorcode) {
            ...
            if(trim($ProductCode) !== '' || !empty(trim($ProductCode))) {
                ...
                $n_data = $this->util->fetchproduct($user_u->id, $product);//will return JSON.
                //var_dump($data);
                $n_data = json_decode($n_data, true);
                
                if(isset($n_data['records'])){
                    $this->logger->write("Api : fetchproduct() : The fetch returned some/zero records", 'r');
                    if ($n_data['records']) {
                        ...
                        $product['erpquantity'] = $this->util->removeunitsfromqty($ErpQty);
                        ...
                        if ($pdct->dry()) {
                            ...
                            $pdct_status = $this->util->createproduct($product);
                            ...
                            $pdct_status = $this->util->updateproduct($product);
                        }
                        ...
                        $q_data = $this->util->stockquery($user_u->id, $branch->uraid, $ProductCode); //will return JSON
    ...
    function uploadproduct(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->uploadproduct($user_u->id, $product, $altunits);//will return JSON.
    ...
    function uploadinvoice(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $deemedflag = trim($xml->DEEMEDFLAG);
                ...
                if (isset($xml->INVENTORIES->INVENTORY)) {
                    ...
                    foreach ($xml->INVENTORIES->INVENTORY as $obj){
                        ...
                        $taxid = $this->util->getinvoicetaxrate($industrycode, $buyertype, $deemedflag, trim($obj->PRODUCTCODE));
                        
                        if (!$taxid) {
                            $taxid = $this->appsettings['STANDARDTAXRATE'];
                        }
                        
                        //Default the tax rate to STANDARD in case the product has more than one tax rate, e.g WHEAT has both standard and exempt                     
                        if ($this->appsettings['DEFAULT_VAT_APPLICATION_MTD'] == '1') {
                            $taxid = $this->appsettings['STANDARDTAXRATE'];
                        }
                        
                        $this->logger->write("Api : uploadinvoice() : The TAXID is " . $taxid, 'r');
                ...
                $data = $this->util->uploadinvoice($user_u->id, $branch->uraid, $buyer, $invoicedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
                ...
                if (isset($data['returnCode'])){
                    ...
                    if (isset($data['basicInformation'])){
                        ...
                        try{
                            //$this->db->exec(array('UPDATE tblinvoices SET antifakeCode = "' . $antifakeCode . '", einvoiceid = "' . $invoiceId . '", einvoicenumber = "' . $invoiceNo . '", issueddate = "' . $issuedDate . '", issueddatepdf = "' . $issuedDatePdf . '", oriinvoiceid = "' . $oriInvoiceId . '", isinvalid = "' . $isInvalid . '", isrefund = "' . $isRefund . '", issuedtime = "' . $issuedTime . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE id = ' . $id));
                            //$this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                            $this->logger->write("Api : uploadinvoice() : Failed to insert into the table tblinvoices. The error message is " . $e->getMessage(), 'r');
                        }
                    ...
                    if (isset($data['summary'])){
                        ...
                        try{
                            //$this->db->exec(array('UPDATE tblinvoices SET grossamount = ' . $grossAmount . ', itemcount = ' . $itemCount . ', netamount = ' . $netAmount . ', einvoicedatamatrixcode = "' . addslashes($qrCode) . '", taxamount = ' . $taxAmount . ', modecode = "' . $modeCode . '", modename = "' . $modeName . '", grossamountword = "' . addslashes($grossAmountWords) . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE id = ' . $id));
                            //$this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                            $this->logger->write("Api : uploadinvoice() : Failed to insert into the table tblinvoices. The error message is " . $e->getMessage(), 'r');
                        }
                    ...
                    $inv_status = $this->util->createinvoice($invoicedetails, $goods, $taxes, $buyer);
    ...
    function uploadcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                    ...
                    if ($orig_inv->dry()) {
                        ...
                        if(trim($oriinvoiceid) == '' || empty(trim($oriinvoiceid))) {
                            ...
                            if(trim($oriinvoiceno) == '' || empty(trim($oriinvoiceno))) {
                                ...
                                $i_data = $this->util->downloadinvoice($user_u->id, $oriinvoiceno);
                ...
                $data = $this->util->uploadcreditnote($user_u->id, $buyer, $creditnotedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
                ...
                if (isset($data['returnCode'])){
                    ...
                    $inv_status = $this->util->createcreditnote($creditnotedetails, $goods, $taxes, $buyer);
                ...
                $n_data = $this->util->downloadcreditnote($user_u->id, $inv_check->referenceno);//will return JSON.
                //var_dump($data);
                ...
                if(isset($n_data['records'])){
                    ...
                    foreach($n_data['records'] as $elem){
                        ...
                        $i_data = $this->util->downloadinvoice($user_u->id, $elem['invoiceNo']);
                        $i_data = json_decode($i_data, true);
                        ...
                        try{
                            $this->db->exec(array('UPDATE tblcreditnotes SET antifakecode = "' . $antifakeCode . '", einvoiceid = "' . $invoiceId . '", einvoicenumber = "' . $invoiceNo . '", issueddate = "' . $issuedDate . '", issuedtime = "' . $issuedTime . '", issueddatepdf = "' . $issuedDatePdf . '", oriinvoiceid = "' . $oriInvoiceId . '", isinvalid = "' . $isInvalid . '", isrefund = "' . $isRefund . '", grossamount = ' . $grossAmount . ', itemcount = ' . $itemCount . ', netamount = ' . $netAmount . ', einvoicedatamatrixcode = "' . $qrCode . '", taxamount = ' . $taxAmount . ', modecode = "' . $modeCode . '", modename = "' . $modeName . '", grossamountword = "' . $grossAmountWords . '", origrossamount = ' . $oriGrossAmount . ', refundinvoiceno = "' . $refundInvoiceNo . '", approvestatus = "' . $approveStatusCode . '", grossamount = "' . $grossAmount . '", totalamount = "' . $totalAmount . '", issueddate = "' . $refundIssuedDate . '", issuedtime = "' . $refundIssuedDate . '", creditnoteapplicationid = "' . $appId . '", applicationtime = "' . $applicationTime . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE referenceno = "' . $inv_check->referenceno . '"'));
                            $this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                            $this->logger->write("Api : uploadcreditnote() : Failed to update the table tblcreditdebitnotes. The error message is " . $e->getMessage(), 'r');
                        }
    ...
    function uploaddebitnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $data = $this->util->uploaddebitnote($user_u->id, $branch->uraid, $buyer, $debitnotedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
                ...
                if (isset($data['returnCode'])){
                    ...
                    if (isset($data['basicInformation'])){
                        ...
                        try{
                            //$this->db->exec(array('UPDATE tbldebitnotes SET antifakeCode = "' . $antifakeCode . '", edebitnoteid = "' . $debitnoteId . '", edebitnotenumber = "' . $debitnoteNo . '", issueddate = "' . $issuedDate . '", issueddatepdf = "' . $issuedDatePdf . '", oridebitnoteid = "' . $oriInvoiceId . '", isinvalid = "' . $isInvalid . '", isrefund = "' . $isRefund . '", issuedtime = "' . $issuedTime . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE id = ' . $id));
                            //$this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                            $this->logger->write("Api : uploaddebitnote() : Failed to insert into the table tbldebitnotes. The error message is " . $e->getMessage(), 'r');
                        }
                    ...
                    if (isset($data['summary'])){
                        ...
                        try{
                            //$this->db->exec(array('UPDATE tbldebitnotes SET grossamount = ' . $grossAmount . ', itemcount = ' . $itemCount . ', netamount = ' . $netAmount . ', edebitnotedatamatrixcode = "' . addslashes($qrCode) . '", taxamount = ' . $taxAmount . ', modecode = "' . $modeCode . '", modename = "' . $modeName . '", grossamountword = "' . addslashes($grossAmountWords) . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE id = ' . $id));
                            //$this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                            $this->logger->write("Api : uploaddebitnote() : Failed to insert into the table tbldebitnotes. The error message is " . $e->getMessage(), 'r');
                        }
                    ...
                    $inv_status = $this->util->createdebitnote($debitnotedetails, $goods, $taxes, $buyer);
        ...
        if ($this->errorcode) {
            ...
            if ($vchtype == 'Credit Note') {
                    ...
                    if(isset($n_data['records'])){
                        ...
                        foreach($n_data['records'] as $elem){
                            ...
                            try{
                                ...
                                $this->logger->write("Api : queryinvoice() : Failed to update the table tblcreditdebitnotes. The error message is " . $e->getMessage(), 'r');
                            }
                ...
                if($inv_check->dry ()){
                    ...
                    $i_data = $this->util->downloadinvoice($user_u->id, $invoicenumber);
                    $i_data = json_decode($i_data, true);
                    ...
                    if ($invoicenumber == $invoiceNo) {
                        try{
                            $this->db->exec(array('UPDATE tbldebitnotes SET oriinvoiceno = "' . $oriInvoiceNo . '", antifakecode = "' . $antifakeCode . '", einvoiceid = "' . $invoiceId . '", einvoicenumber = "' . $invoiceNo . '", issueddate = "' . $issuedDate . '", issuedtime = "' . $issuedTime . '", issueddatepdf = "' . $issuedDatePdf . '", oriinvoiceid = "' . $oriInvoiceId . '", isinvalid = "' . $isInvalid . '", isrefund = "' . $isRefund . '", grossamount = ' . $grossAmount . ', itemcount = ' . $itemCount . ', netamount = ' . $netAmount . ', einvoicedatamatrixcode = "' . $qrCode . '", taxamount = ' . $taxAmount . ', modecode = "' . $modeCode . '", modename = "' . $modeName . '", grossamountword = "' . $grossAmountWords . '", origrossamount = ' . $oriGrossAmount . ', grossamount = "' . $grossAmount . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE einvoicenumber = "' . $invoicenumber . '"'));
                            $this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
                ...
                if($inv_check->dry ()){
                    ...
                    if ($invoicenumber == $invoiceNo) {
                        try{
                            $this->db->exec(array('UPDATE tblinvoices SET antifakecode = "' . $antifakeCode . '", einvoiceid = "' . $invoiceId . '", einvoicenumber = "' . $invoiceNo . '", issueddate = "' . $issuedDate . '", issuedtime = "' . $issuedTime . '", issueddatepdf = "' . $issuedDatePdf . '", isinvalid = "' . $isInvalid . '", isrefund = "' . $isRefund . '", grossamount = ' . $grossAmount . ', itemcount = ' . $itemCount . ', netamount = ' . $netAmount . ', einvoicedatamatrixcode = "' . $qrCode . '", taxamount = ' . $taxAmount . ', modecode = "' . $modeCode . '", modename = "' . $modeName . '", grossamountword = "' . $grossAmountWords . '", grossamount = "' . $grossAmount . '", modifieddt = NOW(), modifiedby = ' . $this->userid . ' WHERE einvoicenumber = "' . $invoicenumber . '"'));
                            $this->logger->write($this->db->log(TRUE), 'r');
                        } catch (Exception $e) {
    ...
    function printinvoice(){
        ...
        if ($this->errorcode) {
            ...
            $this->logger->write("Api : printinvoice() : Priting of invoice started", 'r');
            
            $id = trim($this->params['invoiceno']);
            ...
            try{
                ...
                $temp = $this->db->exec(array('SELECT item, FORMAT(qty, 2) qty, unitofmeasure, FORMAT(unitprice, 2) unitprice, FORMAT(total, 2) total, taxcategory, unitofmeasurename, discounttotal FROM tblgooddetails WHERE groupid = COALESCE(' . $invoice->gooddetailgroupid . ', NULL) ORDER BY inserteddt ASC'));
                ...
                $this->logger->write("Api : printinvoice() : The operation to retrieve goods was not successfull. The error messages is " . $e->getMessage(), 'r');
                $goods = array(
                    "0" => array()
                );
            }
            ...
            try{
                ...
                $this->logger->write("Api : printinvoice() : The operation to retrieve taxes was not successfull. The error messages is " . $e->getMessage(), 'r');
                $taxes = array(
                    "0" => array()
                );
            }
            ...
            try{
                ...
                $this->logger->write("Api : printinvoice() : The operation to retrieve payments was not successfull. The error messages is " . $e->getMessage(), 'r');
                $payments = array(
                    "0" => array()
                );
            }
            ...
            $pdf->SetAuthor('e-TaxWare Team');
            $pdf->SetTitle('e-INVOICE/TAX INVOICE');
            $pdf->SetSubject('EFRIS');
            
            // set default header data
            //$pdf->SetHeaderData(NULL, NULL, 'e-INVOICE/TAX INVOICE', NULL);
            ...
            $html = '
            	...
            	<tbody>
            	<tr>
            	<td><h1>INVOICE<strong> #' . $invoice->erpinvoiceid . '</strong></h1></td>
            	<td align="right"><img src="logo.jpg" height="60px"/><br/>
            	' . $org->legalname . '<br/>
            	' . $org->address . '<br/>
                <strong> TIN: ' . $org->tin . '</strong><br/>
            	<strong>' . $org->mobilephone . '</strong> | <strong>' . $org->emailaddress . '</strong>
            	</td>
            ...
            $html .= '
            	...
            	<tbody>
            	<tr>
            	<td><strong>' . $buyer->legalname . '</strong><br/>
            	' . $buyer->address . '<br/>
                TIN: ' . $buyer->tin . '<br/>
                ' . $buyer->mobilephone . ' | ' . $buyer->emailaddress . '
            	</td>
            	<td align="right">
            	<strong>Fiscal Doc. Number: ' . $invoice->antifakecode . '</strong><br/>
            	Invoice Number: ' . $invoice->einvoicenumber . '<br/>
            	Issue Date: ' . $invoice->issuedtime . '
            	</td>
            	</tr>
            	</tbody>
            	</table>
            ...
            $html .= '
            	<h1 style="text-align:center"><strong>Goods/Service Details</strong></h1>
                <table>
            	<thead>
            ...
            foreach ($goods as $obj) {
                $html .= '
            		<tr>
            		<td style="border-bottom: 1px solid #222">' . $obj['item'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['qty'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['unitofmeasurename'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['unitprice'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['total'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['taxcategory'] . '</td>
            		</tr>
            		';
            }
            ...
            $html .= '
            	<h1 style="text-align:center"><strong>Tax Details</strong></h1>
                <table>
            	<thead>
            	<tr style="font-weight:bold;">
            	<th>Tax Category</th>
            ...
            foreach ($taxes as $obj) {
                $html .= '
            		<tr>
            		<td style="border-bottom: 1px solid #222">' . $obj['taxdescription'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['netamount'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['taxamount'] . '</td>
            		<td style="border-bottom: 1px solid #222">' . $obj['grossamount'] . '</td>
            		</tr>
            		';
            }
            
            $html .= '	</tbody></table>';
            $html .= '	<h1 style="text-align:center"><strong>Summary</strong></h1>';
            ...
            $html .= '
            	...
            	<td>' . $invoice->netamount2 . '<br/>
            	' . $invoice->taxamount2 . '<br/>
                ' . $invoice->grossamount2 . '<br/>
                ' . $invoice->currency . '<br/>
                ' . $invoice->itemcount . '<br/>
                ' . $invoice->modename . '<br/>
                ' . $invoice->remarks . '<br/>
            	</td>
            	</tr>
            ...
            $pdf->write2DBarcode($invoice->einvoicedatamatrixcode, 'QRCODE,L', 160, 230, 25, 25, $style, 'N');
            //$pdf->Text(20, 25, 'QR CODE');
    ...
    function voidcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                //Submit the cencel request
                $data = $this->util->voidcreditnote($user_u->id, $inv_check->creditnoteapplicationid, $inv_check->oriinvoiceno, $inv_check->referenceno);
                $data = json_decode($data, true);
                /**
