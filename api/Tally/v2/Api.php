...
Class Api{
    ...
    function sendmail(){
        ...
        if ($recipientemail && $body) {
            try {
                ...
                $mail->Username = 'admin@etaxware.com';
                
                //Password to use for SMTP authentication
                $mail->Password = 'Admin$1234';
                
                //Set who the message is to be sent from
                $mail->setFrom('admin@etaxware.com', 'e-TaxWare App');
    ...
    function validatetin(){
        $operation = NULL; //tblevents
        $permission = 'VALIDATETIN'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->querytaxpayer($this->userid, $tin);//will return JSON.
            //var_dump($data);
    ...
    function checktaxpayer(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->checktaxpayer($this->userid, $tin, $commodity);//will return JSON.
            //var_dump($data);
    ...
    function currencyquery(){
        $operation = NULL; //tblevents
        $permission = 'CURRENCYQUERY'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->fetchcurrencyrates($this->userid); // will return JSON.
            // var_dump($data);
            $data = json_decode($data, true);
    ...
    function stockin(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->stockin($this->userid, $productcode, $batchno, $qty, $suppliertin, $suppliername, $stockintype, $productiondate, $unitprice);//will return JSON.
    ...
    function batchstockin(){
        $operation = NULL; //tblevents
        $permission = 'BATCHSTOCKIN'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $vch_check = new DB\SQL\Mapper($this->db, 'tblgoodsstockin');
            ...
            if($vch_check->dry ()){
                ...
                $data = $this->util->batchstockin($this->userid, $products, $batchno, $suppliertin, $suppliername, $stockintype, $productiondate);//will return JSON.
    ...
    function stockout(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->stockout($this->userid, $productcode, $batchno, $qty, $adjustmenttype, $remarks);
    ...
    function fetchproduct(){
        ...
        if ($this->errorcode) {
            ...
            if(trim($ProductCode) !== '' || !empty(trim($ProductCode))) {
                ...
                $n_data = $this->util->fetchproduct($this->userid, $product);//will return JSON.
                //var_dump($data);
    ...
    function uploadproduct(){
        ...
        if ($this->errorcode) {
            ...
            $data = $this->util->uploadproduct($this->userid, $product, $altunits);//will return JSON.
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $invoicedetails = array(
                    ...
                    'antifakecode' => NULL,
                    'deviceno' => trim($tcsdetails->deviceNo),
                    'issueddate' => NULL,
                    'issuedtime' => NULL,
                ...
                $buyer = array(
                    ...
                    'referenceno' => trim($xml->VOUCHERREF) == ''? trim($xml->BUYERREFERENCENO) : trim($xml->VOUCHERREF) . trim($xml->VOUCHERNUMBER),
                    'datasource' => $this->appsettings['WESERVICEDS']
                );
                ...
                $data = $this->util->uploadinvoice($this->userid, $buyer, $invoicedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
    ...
    function uploadcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $data = $this->util->uploadcreditnote($this->userid, $buyer, $creditnotedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
                ...
                $n_data = $this->util->downloadcreditnote($this->userid, $inv_check->referenceno);//will return JSON.
                //var_dump($data);
                ...
                if(isset($n_data['records'])){
                    ...
                    foreach($n_data['records'] as $elem){
                        ...
                        $i_data = $this->util->downloadinvoice($this->userid, $elem['invoiceNo']);
                        $i_data = json_decode($i_data, true);
    ...
    function uploaddebitnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $data = $this->util->uploaddebitnote($this->userid, $buyer, $debitnotedetails, $goods, $payments, $taxes);
                $data = json_decode($data, true);
    ...
    function queryinvoice(){
        $operation = NULL; //tblevents
        $permission = 'QUERYINVOICE'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            if ($vchtype == 'Credit Note') {
                ...
                if($inv_check->dry ()){
                    ...
                    $i_data = $this->util->downloadinvoice($this->userid, $invoicenumber);
                    $i_data = json_decode($i_data, true);
    ...
    function printinvoice(){
        $operation = NULL; //tblevents
        $permission = 'PRINTINVOICE'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
    ...
    function voidcreditnote(){
        ...
        if ($this->errorcode) {
            ...
            if($inv_check->dry ()){
                ...
                $data = $this->util->voidcreditnote($this->userid, $inv_check->creditnoteapplicationid, $inv_check->oriinvoiceno, $inv_check->referenceno);
                $data = json_decode($data, true);
                /**
