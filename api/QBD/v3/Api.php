...
/**
 * @name Api.php
 * @desc This file is part of the etaxware-api app. This is the API version 3
 * @date: 05-01-2024
 * @file: Api.php
 * @path: ./api/QBD/v3/Api.php
 * @author: francis lubanga <fl@dalytics.co>
...
Class Api{
    ...
    protected $subject;
    protected $ccrecipientemail = 'fl@dalytics.co'; 
    ...
    function validatetin(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                $ninBrn = '';
                $legalName = '';
                $businessName = '';
                $contactNumber = '';
    ...
    /**
     *	@name uploadcommoditycode
     *  @desc upload a commodity category code
     *	@return string response
     *	@param NULL
     **/
    function uploadcommoditycode(){
        $operation = NULL; //tblevents
        $permission = 'UPLOADCOMMODITYCODES'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $commodityCatCode = '';
            $commodityCatName = '';
            //$commodityCode = trim($json['COMMODITYCODE']);
            $commodityId = trim($json['COMMODITYID']);
            $commodityName = trim($json['COMMODITYNAME']);
            
            $this->logger->write("Api : uploadcommoditycode() : The userid is: " . $this->userid_u, 'r');
            $this->logger->write("Api : uploadcommoditycode() : The commodityName is: " . $commodityName, 'r');
            
            $this->logger->write("Api : uploadcommoditycode() : Checking permissions", 'r');
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                try{
                    ...
                    if ($commodityName) {
                        $commoditycategory->getByName($commodityName);
                        $this->logger->write($this->db->log(TRUE), 'r');
                        
                        if ($commoditycategory->dry()) {
                            $this->message = "The commodity does not have an equivalent commodity category.";
                            $this->code = "0896";
                        } else {
                            $this->logger->write("Api : uploadcommoditycode() : The commodity has an equivalent commodity category", 'r');
                            $commodityCatCode = $commoditycategory->commoditycode;
                            $commodityCatName = $commoditycategory->commodityname;
                            
                            if ($commodityId) {
                                $this->db->exec(array('UPDATE tblcommoditycategories SET erpcode = ' . $commodityId .
                                    ', modifieddt = "' .  date('Y-m-d H:i:s') .
                                    '", modifiedby = ' . $this->userid_u .
                                    ' WHERE TRIM(commodityname) = "' . addslashes($commodityName) . '"'));
                                
                                $this->message = "The operation to upload the commodity code was successful.";
                                $this->code = "000";
                            } else {
                                $this->message = "The commodity Id was not provided.";
                                $this->code = "0899";
                            }
                             
                        }
                    } else {
                        $this->message = "The commodity name was not provided.";
                        $this->code = "0897";
                    }
                ...
                $this->logger->write("Api : uploadcommoditycode() : The user is not allowed to perform this function", 'r');
                $this->message = "The user is not allowed to perform this function";
                $this->code = '0099';
            }
            
            $this->util->createerpauditlog($this->userid_u, $permission, $windowsuser, $ipaddress, $macaddress, $systemname, json_encode($json), NULL, NULL, NULL, $this->code, $this->message);
            
            if ($this->code !== '000') {
                $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to upload a commodity code failed with the following details;</p>
                                            <p>Commodity: <b>' . $commodityName . '</b></p>
        ...
        $this->response = array(
            ...
            "data" => array(
                "COMMODITYCATEGORYCODE" => $commodityCatCode,
                "COMMODITYCATEGORYNAME" => $commodityCatName
            )
        );
    ...
    function uploadcustomer(){
        ...
        if ($this->errorcode) {
            ...
            $fullName = trim($json['FULLNAME']);
            $customerid = "";
            $customername = "";
            
            $this->logger->write("Api : uploadcustomer() : The userid is: " . $this->userid_u, 'r');
            $this->logger->write("Api : uploadcustomer() : The fullName is: " . $fullName, 'r');
            
            $this->logger->write("Api : uploadcustomer() : Checking permissions", 'r');
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                try {
                    $this->logger->write("Api : uploadcustomer() : Customer Name: " . trim($json['FULLNAME']), 'r');
                    
                    
                    $erpcustomerid = trim($json['CUSTOMERID']);
                    $erpcustomercode = empty(trim($json['CUSTOMERCODE']))? trim($json['CUSTOMERID']) : trim($json['CUSTOMERCODE']);
                    $legalname = empty(trim($json['FULLNAME']))? trim($json['BUSINESSNAME']) : trim($json['FULLNAME']);
                    $businessname = empty(trim($json['BUSINESSNAME']))? trim($json['FULLNAME']) : trim($json['BUSINESSNAME']);
                    
                    if(trim($json['MOBILEPHONE'])){
                        $mobilephone = trim($json['MOBILEPHONE']);
                        $cust['mobilephone'] = $mobilephone;
                    }
                    
                    if(trim($json['EMAIL'])){
                        $emailaddress = trim($json['EMAIL']);
                    ...
                    if(trim($json['CUSTOMERTYPE'])){
                        $buyerType = trim($json['CUSTOMERTYPE']);
                        $cust['type'] = $this->util->mapbuyertypecode(trim($buyerType));
                    }
                    
                    $this->logger->write("Api : uploadcustomer() : Mobile: " . $mobilephone, 'r');
                    $this->logger->write("Api : uploadcustomer() : Email: " . $emailaddress, 'r');
                    ...
                    if ($erpcustomercode && $legalname) {
                        ...
                        if ($customer->dry()) {
                            $this->logger->write("Api : uploadcustomer() : The customer does not exist", 'r');
                            $cust_status = $this->util->createcustomer($cust, $this->userid_u);
                            
                            if ($cust_status) {
                                $this->logger->write("Api : uploadcustomer() : The customer " . $cust['legalname'] . " was created.", 'r');
                                
                                $this->message = "The customer was created on e-TaxWare";
                                $this->code = '000';
                            } else {
                                $this->logger->write("Api : uploadcustomer() : The customer " . $cust['legalname'] . " was NOT created.", 'r');
                                
                                $this->message = "The customer was not created e-TaxWare";
                                $this->code = '0096';
                            }
                        } else {
                            $this->logger->write("Api : uploadcustomer() : The customer exists", 'r');
                            $cust['id'] = $customer->id;
                            
                            $customerid = $customer->id;
                            $customername = $customer->legalname;
                            
                            $cust_status = $this->util->updatecustomer($cust, $this->userid_u);
                            
                            if ($cust_status) {
                                $this->logger->write("Api : uploadcustomer() : The customer " . $cust['legalname'] . " was updated.", 'r');
                            } else {
                                $this->logger->write("Api : uploadcustomer() : The customer " . $cust['legalname'] . " was NOT updated.", 'r');
                            }
                            
                            $this->message = "The customer exists on e-TaxWare";
                            $this->code = '000';
                        }
                        
                    } else {
                        $this->logger->write("Api : uploadcustomer() : The customer has no name or code.", 'r');
                        
                        $this->message = "The customer has no name or code";
                        $this->code = '0098';
                    }
                    
                } catch (Exception $e) {
                    $this->logger->write("Api : uploadcustomer() : There was an error when processing Item " . trim($json['FULLNAME']) . ". The error is " . $e->getMessage(), 'r');
                    
                    $this->message = "There was an error when processing Item " . trim($json['FULLNAME']) . ". The error is " . $e->getMessage();
                    $this->code = '0997';
                }
                
            } else {
                $this->logger->write("Api : uploadcustomer() : The user is not allowed to perform this function", 'r');
                $this->message = "The user is not allowed to perform this function";
                $this->code = '0099';
            }
            ...
            if ($this->code !== '000') {
                $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to upload a customer failed with the following details;</p>
                                            <p>Customer: <b>' . $fullName . '</b></p>
                                            <p>Error Code: <b>' . $this->code . '</b></p>
                                            <p>Error Message: <b>' . $this->message . '</b></p>
                                            </br>
                                            <p>Regards,</p>
    ...
    /**
     *	@name uploadsupplier
     *  @desc upload a supplier
     *	@return string response
     *	@param NULL
     **/
    function uploadsupplier(){
        $operation = NULL; //tblevents
        $permission = 'SYNCSUPPLIERS'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $fullName = trim($json['BUSINESSNAME']);
            $supplierid = "";
            $suppliername = "";
            
            $this->logger->write("Api : uploadsupplier() : The userid is: " . $this->userid_u, 'r');
            $this->logger->write("Api : uploadsupplier() : The fullName is: " . $fullName, 'r');
            
            $this->logger->write("Api : uploadsupplier() : Checking permissions", 'r');
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                try {
                    $this->logger->write("Api : uploadsupplier() : Supplier Name: " . trim($json['BUSINESSNAME']), 'r');
                    
                    
                    $erpsupplierid = trim($json['SUPPLIERID']);
                    $erpsuppliercode = trim($json['SUPPLIERCODE']);
                    $legalname = empty(trim($json['FULLNAME']))? trim($json['BUSINESSNAME']) : trim($json['FULLNAME']);
                    $businessname = empty(trim($json['BUSINESSNAME']))? trim($json['FULLNAME']) : trim($json['BUSINESSNAME']);
                    ...
                    $cust['type'] = $this->appsettings['B2BCODE'];
                    
                    $this->logger->write("Api : uploadsupplier() : Mobile: " . $mobilephone, 'r');
                    $this->logger->write("Api : uploadsupplier() : Email: " . $emailaddress, 'r');
                    ...
                    if ($erpsupplierid && $legalname) {
                        
                        $supplier->getBySupplierId($erpsupplierid);
                        
                        if ($supplier->dry()) {
                            $this->logger->write("Api : uploadsupplier() : The supplier does not exist", 'r');
                            $cust_status = $this->util->createsupplier($cust, $this->userid_u);
                            
                            if ($cust_status) {
                                $this->logger->write("Api : uploadsupplier() : The supplier " . $cust['legalname'] . " was created.", 'r');
                                
                                $this->message = "The supplier was created on e-TaxWare";
                                $this->code = '000';
                            } else {
                                $this->logger->write("Api : uploadsupplier() : The supplier " . $cust['legalname'] . " was NOT created.", 'r');
                                
                                $this->message = "The supplier was not created e-TaxWare";
                                $this->code = '0096';
                            }
                        } else {
                            $this->logger->write("Api : uploadsupplier() : The supplier exists", 'r');
                            $cust['id'] = $supplier->id;
                            
                            $supplierid = $supplier->id;
                            $suppliername = $supplier->legalname;
                            
                            $cust_status = $this->util->updatesupplier($cust, $this->userid_u);
                            
                            if ($cust_status) {
                                $this->logger->write("Api : uploadsupplier() : The supplier " . $cust['legalname'] . " was updated.", 'r');
                            } else {
                                $this->logger->write("Api : uploadsupplier() : The supplier " . $cust['legalname'] . " was NOT updated.", 'r');
                            }
                            
                            $this->message = "The supplier exists on e-TaxWare";
                            $this->code = '000';
                        }
                        
                    } else {
                        $this->logger->write("Api : uploadsupplier() : The supplier has no name or code.", 'r');
                        
                        $this->message = "The supplier has no name or code";
                        $this->code = '0098';
                    }
                    
                } catch (Exception $e) {
                    $this->logger->write("Api : uploadsupplier() : There was an error when processing Item " . trim($json['FULLNAME']) . ". The error is " . $e->getMessage(), 'r');
                ...
                $this->logger->write("Api : uploadsupplier() : The user is not allowed to perform this function", 'r');
                $this->message = "The user is not allowed to perform this function";
                $this->code = '0099';
            }
            ...
            if ($this->code !== '000') {
                $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to upload a supplier failed with the following details;</p>
                                            <p>Supplier: <b>' . $fullName . '</b></p>
                                            <p>Error Code: <b>' . $this->code . '</b></p>
                                            <p>Error Message: <b>' . $this->message . '</b></p>
                                            </br>
                                            <p>Regards,</p>
    ...
    function stockin(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                if (trim($suppliertin) == '' || empty($suppliertin)) {
                    ...
                    if (isset($v_data['taxpayer'])){
                        ...
                        $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to increase stock failed with the following details;</p>
            ...
            $this->util->createerpauditlog($this->userid_u, $permission, $windowsuser, $ipaddress, $macaddress, $systemname, json_encode($json), NULL, NULL, $productcode, $this->code, $this->message);
        }
                ...
                if($vch_check->dry ()){
                    ...
                    if((int)$errorCount == 0){
                        ...
                        if (isset($json['INVENTORIES'])) {
                            ...
                            $this->logger->write("Api : batchstockin() : No goods were supplied!", 'r');
    ...
    function stockout(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                $data = json_decode($data, true);
                //$this->logger->write("Api : stockout() : The returned data is : " . $data, 'r');
                ...
                if(isset($data['returnCode'])){
                    ...
                    $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to decrease stock failed with the following details;</p>
                                            <p>Product: <b>' . $productcode . '</b></p>
                                            <p>Error Code: <b>' . $this->code . '</b></p>
                                            <p>Error Message: <b>' . $this->message . '</b></p>
                                            </br>
                ...
                if($vch_check->dry ()){
                    ...
                    if (isset($json['INVENTORIES'])) {
                        foreach ($json['INVENTORIES'] as $obj){
                            ...
                            /**
                             * Modification Date: 2024-01-16
                             * Modified By: Francis Lubanga
                             * Description: 663 - quantity:Cannot be negative!
                             * */
                            $qty = intval(trim($obj['QTY']));
                            $amount = floatval(trim($obj['AMOUNT']));
                            $this->logger->write("Api : batchstockout() : The quantity for product code " . trim($obj['PRODUCTCODE']) . " is: " . $qty, 'r');
                            $this->logger->write("Api : batchstockout() : The amount for product code " . trim($obj['PRODUCTCODE']) . " is: " . $amount, 'r');
                            
                            if ($qty < 0){ //If the quantity is negative, then drop the negative
                                $obj['QTY'] = abs($qty);
                            }
                            
                            if (trim($obj['RATE']) == '' || empty($obj['RATE'])) {
                                if ($qty <> 0){
                                    $u_price = abs($amount)/abs($qty);
                                } else {
                                    $u_price = 1;
                                } 
                                
                                $obj['RATE'] = abs($u_price);
                            }
                        ...
                        $this->logger->write("Api : batchstockout() : No goods were supplied!", 'r');
                        $this->message = "No goods were supplied!";
                        $this->code = '999';
                    ...
                    if(isset($n_data['records'])){
                        ...
                        if ($n_data['records']) {
                            ...
                            if ($pdct->dry()) {
                                ...
                                $product['description'] = "This product was created by the QBD api";
                                ...
                                $product['description'] = "This product was updated by the QBD api";
                            ...
                            if (isset($q_data['stock'])){
                                ...
                                $body = '<html><body>
                                            <p>Hello,</p></br>
                                            <p>Your request to fetch a product failed with the following details;</p>
                                            <p>Product: <b>' . $ProductCode . '</b></p>
                                            <p>Error Code: <b>' . $q_data['returnCode'] . '</b></p>
                                            <p>Error Message: <b>' . $q_data['returnMessage'] . '</b></p>
                                            </br>
                                            <p>Regards,</p>
                                            <p>e-TaxWare Team</p>
                                            </body></html>';
                                ...
                                $body = '<html><body>
                                            ...
                                            <p>Your request to fetch a product failed with the following details;</p>
                                            <p>Product: <b>' . $ProductCode . '</b></p>
                                            <p>Error Code: <b>-999</b></p>
                                            <p>Error Message: <b>The operation to query the taxpayer was not successful</b></p>
                                            </br>
                                            <p>Regards,</p>
                                            <p>e-TaxWare Team</p>
                                            </body></html>';
    ...
    function uploadproduct(){
        ...
        if ($this->errorcode) {
            ...
            $this->logger->write("Api : uploadproduct() : The product name is " . $ProductName, 'r');
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                if (empty(trim($ProductCode)) || empty(trim($ProductName)) || empty(trim($ProductCommodityCode))) {
                    $this->logger->write("Api : uploadproduct() : One or more mandatory fields not supplied", 'r');
                    $this->message = "One or more mandatory fields not supplied";
                    $this->code = '0098';
                } else {
                    $altunits = array();
                    ...
                    $product = array(
                        ...
                        'commoditycategorycode' => $this->util->mapcommodity_v3(trim($ProductCommodityCode)),
                        'hasexcisetax' => strtoupper(trim($ProductHasExciseDuty)) == 'NO' || empty(trim($ProductHasExciseDuty)) || trim($ProductHasExciseDuty) == ''? '102' : '101',
                        'description' => NULL,
                        'stockprewarning' => empty($ProductStockPrewarning)? '0' : $ProductStockPrewarning,
                        ...
                        'excisedutycode' => empty(trim($ProductExciseDutyCode))? '' : trim($ProductExciseDutyCode),
                        'uraquantity' => 0,
                        'erpquantity' => 0,
                        'purchaseprice' => 0,
                        'stockintype' => NULL,
                    ...
                    if (empty($data)) {
                        ...
                        $this->logger->write("Api : uploadproduct() : The product " . $product['code'] . " was not uploaded successfully", 'r');
                        //$this->logger->write("Api : uploadproduct() : The returned payload is: " . json_encode($data), 'r');
     ...
     **/
        ...
        if ($this->errorcode) {
			...
			//$json = json_decode($this->xml, true);
...
// Get inventory data
            ...
            if ($this->userpermissions[$permission]) {
                ...
                if($inv_check->dry ()){
                    ...
                    $this->logger->write("Api : uploadinvoice() : The voucher narration is: " . trim($json['VOUCHERNARRATION']), 'r');
                    
                    $remarks = '';
                    
                    if (isset($json['REASONS']['REASON'])) {
                    ...
                    $invoicedetails = array(
                        ...
                        'invoiceindustrycode' => $this->util->mapindustrycode(trim($json['INDUSTRYCODE'])),
                        'einvoiceid' => NULL,
                        'einvoicenumber' => NULL,
                    ...
                    $buyer = array(
                        ...
                        'referenceno' => trim($json['VOUCHERNUMBER']) == ''? trim($json['BUYERREFERENCENO']) : trim($json['VOUCHERREF']) . trim($json['VOUCHERNUMBER']),
                        'datasource' => $this->appsettings['WESERVICEDS']
                    );
                     ...
                     */
                    //$deemedflag = 'NO';
                    $deemedflag = trim($json['DEEMEDFLAG']) == ''? 'NO' : strtoupper(trim($json['DEEMEDFLAG']));
                                        
                    //$vatapplicationlevel = trim($xml->VATAPPLICATIONLEVEL);//Ledger
                    $pricevatinclusive = trim($json['PRICEVATINCLUSIVE']);//No
                    //$defaultvatrate = trim($xml->DEFAULTVATRATE);//18
                    $buyertype = $this->util->mapbuyertypecode(trim($json['BUYERTYPE']));
                    $industrycode = $this->util->mapindustrycode(trim($json['INDUSTRYCODE']));
					...
					//$data = json_decode($this->xml, true);
					$json = json_decode($this->json, TRUE);
					//$inventory = $data['INVENTORIES'];
					
					
					 // print_r($json['INVENTORIES']);
					
					$objj = $json['INVENTORIES'];
				
                    if (isset($json['INVENTORIES'])) {
                        $this->logger->write("Api : uploadinvoice() : This is an inventory/goods invoice", 'r');
                        
                        //foreach ($xml->INVENTORIES->INVENTORY as $obj){
							for ($i = 0; $i < count($objj); $i++) {
								 $obj = $objj['INVENTORY'];
								 
								 	//print_r($objj['INVENTORY']['PRODUCTCODE']);
								 
								 //echo 'hello---'.$obj['PRODUCTCODE'];
								// echo '/////';
    ...
    function uploadinvoicenn(){
        ...
        if ($this->errorcode) {
            ...
            if (empty(trim($json['VOUCHERNUMBER'])) || empty(trim($json['INDUSTRYCODE'])) || empty(trim($json['PRICEVATINCLUSIVE']))) {
                $this->logger->write("Api : uploadinvoice() : One or more mandatory fields was not specified.", 'r');
                $this->message = "One or more mandatory fields was not specified.!";
                $this->code = '-999';
                //break;//exit loop
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                if($inv_check->dry ()){
                    ...
                    $buyer = array(
                        ...
                        'emailaddress' => trim($json['BUYEREMAIL']),
                        'placeofbusiness' => trim($json['BUYERPLACEOFBUSI']),
                        'type' => $this->util->mapbuyertypecode(trim(trim($json['BUYERTYPE']))),
                        'citizineship' => trim($json['BUYERCITIZENSHIP']),
                        'sector' => trim($json['BUYERSECTOR']),
                    ...
                    /*
                     * Retrieve buyer details from e-taxware
                     */
                    $this->logger->write("Api : uploadinvoice() : The buyer reference is: " . $json['BUYERREFERENCENO'], 'r');
                    
                    if (trim($json['BUYERREFERENCENO']) == '' || empty($json['BUYERREFERENCENO'])) {
                        $this->logger->write("Api : uploadinvoice() : The buyer details were not provided!", 'r');
                        
                    } else {
                        $customer = new customers($this->db);
                        $customer->getByCustomerId(trim($json['BUYERREFERENCENO']));
                        
                        if ($customer->dry()) {
                            $this->logger->write("Api : uploadinvoice() : The customer does not exist", 'r');
                            
                        } else {
                            $this->logger->write("Api : uploadinvoice() : The customer exists", 'r');
                            $json['BUYERTIN'] = $customer->tin;
                    ...
                    if (isset($json['INVENTORIES'])) {
                        foreach ($json['INVENTORIES'] as $obj){
                            ...
                            /**
                             * Determine the tax code relevant to the product. QBD will always send a TAXCODE
                             */
                            /*
                        ...
                        if (sizeof($goods) > 0) {
                            ...
                            if (isset($data['returnCode'])){
                                ...
                                if (trim($data['returnCode']) == '2253') {
                                    $this->logger->write("Api : uploadinvoice() : The seller's reference is already in EFRIS. We are going to go ahead and query for the details in EFRIS.", 'r');
                                    
                                    $data_n = $this->util->invoiceinquiry($this->userid_u, NULL, NULL, NULL, NULL, $vchnumber, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
                                    $data_n = json_decode($data_n, true);
                                    
                                    if (isset($data_n['returnCode'])){
                                        $this->logger->write("Api : uploadinvoice() : The operation to query the invoice from EFRIS was not successful. The error message is " . $data_n['returnMessage'], 'r');
                                        $this->message = $data_n['returnMessage'];
                                        $this->code = $data_n['returnCode'];
                                    } else {
                                        $this->logger->write("Api : uploadinvoice() : The operation to query the invoice from EFRIS was successful.", 'r');
                                        /*
                                         {
                                         "page": {
                                         "pageNo": "1",
                                        ...
                                        if(isset($data_n['records'])){
                                            if ($data_n['records']) {
                                                foreach($data_n['records'] as $elem){
                                                    //$invoiceId = $elem['id']; //3257429764295992735
                                                    $invoiceNo = $elem['invoiceNo']; //3120012276043
                                                    //$oriInvoiceId = $elem['oriInvoiceId'];//3257429764295992735
                                                    //$oriInvoiceNo = $elem['oriInvoiceNo'];//3120012276043
                                                    
                                                    $i_data = $this->util->downloadinvoice($this->userid_u, $invoiceNo);
                                                    $i_data = json_decode($i_data, true);
                                                    
                                                    if (isset($i_data['returnCode'])){
                                                        $this->logger->write("Api : uploadinvoice() : The operation to download the invoice details from EFRIS was not successful. The error message is " . $i_data['returnMessage'], 'r');
                                                        $this->message = $i_data['returnMessage'];
                                                        $this->code = $i_data['returnCode'];
                                                    } else {
                                                        $this->logger->write("Api : uploadinvoice() : The operation to download the invoice details from EFRIS was successful.", 'r');
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                if($inv_check->dry ()){
                    ...
                    $creditnotedetails = array(
                         ...
                         * Author: francis.lubanga@gmail.com
                         * Date: 2024-01-05
                         * Description: ERPUSER might be empty! Handle it!
                         */
                        'operator' => trim($json['ERPUSER']), 
                        'currency' => $this->util->getcurrency(trim($json['CURRENCY'])),
    ...
    function queryinvoice(){
        ...
        if ($this->errorcode) {
            ...
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                ...
                if (trim($erpinvoiceid) == '' || empty($erpinvoiceid)) {
                    $this->logger->write("Api : queryinvoice() : There was no voucher number supplied!", 'r');
                    $this->message = 'There was no voucher number supplied!';
                    $this->code = '-999';
                    ...
                    $body = '<html><body>
                                <p>Hello,</p></br>
                                <p>Your request to query an invoice failed with the following details;</p>
                                <p>Number: <b>' . $erpinvoiceid . '</b></p>
                                <p>Error Code: <b>' . $this->code . '</b></p>
                                <p>Error Message: <b>' . $this->message . '</b></p>
                                </br>
                                <p>Regards,</p>
                    ...
                    $this->util->createerpauditlog($this->userid_u, $permission, $windowsuser, $ipaddress, $macaddress, $systemname, json_encode($json), $erpinvoiceid, $erpinvoiceno, NULL, $this->code, $this->message);
                ...
                if ($vchtype == 'Credit Note' || $vchtype == 'Credit Memo') {
                    $inv_check = new DB\SQL\Mapper($this->db, 'tblcreditnotes');
                ...
                } elseif ($vchtype == 'Sales' || stripos($vchtype, 'Sales') || $vchtype == 'Invoice'){
                ...
                } elseif ($vchtype == 'Imported Service Invoice' || stripos($vchtype, 'Imported Service')){
                    $this->logger->write("Api : queryinvoice() : Querying invoice started", 'r');
                    $inv_check = new DB\SQL\Mapper($this->db, 'tblimportedservices');
                    $inv_check->load(array('TRIM(erpinvoiceid)=?', $erpinvoiceid));
                    ...
                    if($inv_check->dry ()){
                        ...
                        if ($invoicenumber == $invoiceNo) {
                            try{
                                ...
                                $this->db->exec(array('UPDATE tblimportedservices SET antifakecode = "' . $antifakeCode .
                                    '", einvoiceid = "' . $invoiceId .
                                    '", einvoicenumber = "' . $invoiceNo .
                                    '", issueddate = "' . $issuedDate .
                                    '", issuedtime = "' . $issuedTime .
                                ...
                                $this->logger->write("Api : queryinvoice() : Failed to update the table tblimportedservices. The error message is " . $e->getMessage(), 'r');
                            }
                        }
     ...
     *  @desc upload imported service
     *	@return string response
     *	@param NULL
     **/
    function uploadimportedservice(){
        $operation = NULL; //tblevents
        $permission = 'UPLOADIMPORTEDSERVICE'; //tblpermissions
        $event = NULL; //tblevents
        $eventnotification = NULL; //tbleventnotifications
        ...
        if ($this->errorcode) {
             ...
             * 2.4 payments
             * 2.5 taxes
             * 3. Send the items in [1] to the uploadimportedservice API
             */
            ...
            if (empty(trim($json['VOUCHERNUMBER'])) || empty(trim($json['INDUSTRYCODE'])) || empty(trim($json['PRICEVATINCLUSIVE']))) {
                $this->logger->write("Api : uploadimportedservice() : One or more mandatory fields was not specified.", 'r');
                $this->message = "One or more mandatory fields was not specified.!";
                $this->code = '-999';
                //break;//exit loop
                ...
                $body = '<html><body>
                                                <p>Hello,</p></br>
                                                <p>Your request to upload an imported service invoice failed with the following details;</p>
                                                <p>Number: <b>' . $vchnumber . '</b></p>
                                                <p>Error Code: <b>' . $this->code . '</b></p>
                                                <p>Error Message: <b>' . $this->message . '</b></p>
                                                </br>
            ...
            $this->logger->write("Api : uploadimportedservice() : The userid is: " . $this->userid_u, 'r');
            $this->logger->write("Api : uploadimportedservice() : Checking permissions", 'r');
            if ($this->userpermissions[$permission] || trim($this->appsettings['ERP_USER_AUTH_SWITCH']) == '0') {
                $branch = new branches($this->db);
                $branch->getByID($this->userbranch_u);
                ...
                if($inv_check->dry ()){
                    $this->logger->write("Api : uploadimportedservice() : The invoice does not exist on e-TW. Proceed and upload", 'r');
                    ...
                    $invoicedetails = array(
                        ...
                        'issuedtime' => NULL,
                        /**
                         * @desc Some ERPs vouchers, like the Bill & PO in QBD do not provide for an ERP user in the API. Handle this! Idea: Use the window user perharps.
                         * @author francis.lubanga@gmail.com
                         * @date 2024-01-16
                         *
                         */
                        ...
                        'invoicekind' => ($this->vatRegistered == 'Y')? "1" : "2",
                        'datasource' => $this->appsettings['WESERVICEDS'],
                        'invoiceindustrycode' => "104", //104 - Imported Service
                        'einvoiceid' => NULL,
                        'einvoicenumber' => NULL,
                        'einvoicedatamatrixcode' => NULL,
                        'isbatch' => '0',
                    ...
                    //We are using the buyer object to hold details of the foreign supplier
                    $importedseller = array(
                        'tin' => trim($json['BUYERTIN']),
                        'ninbrn' => trim($json['BUYERNINBRN']),
                        'PassportNum' => trim($json['BUYERPASSPORTNUM']),
                        'legalname' => trim($json['BUYERLEGALNAME']),
                    ...
                    /*
                     * Retrieve supplier details from e-taxware
                     */
                    $this->logger->write("Api : uploadimportedservice() : The supplier reference is: " . $json['BUYERREFERENCENO'], 'r');
                    
                    if (trim($json['BUYERREFERENCENO']) == '' || empty($json['BUYERREFERENCENO'])) {
                        $this->logger->write("Api : uploadimportedservice() : The supplier details were not provided!", 'r');
                        
                    } else {
                        $supplier = new suppliers($this->db);
                        $supplier->getBySupplierId(trim($json['BUYERREFERENCENO']));
                        
                        if ($supplier->dry()) {
                            $this->logger->write("Api : uploadimportedservice() : The supplier does not exist", 'r');
                            
                        } else {
                            $this->logger->write("Api : uploadimportedservice() : The supplier exists", 'r');
                            $json['BUYERTIN'] = $supplier->tin;
                            $importedseller['tin'] = $supplier->tin;
                        }
                    }
                    ...
                    if (trim($json['BUYERTIN']) == '' || empty($json['BUYERTIN'])) {
                        $this->logger->write("Api : uploadimportedservice() : The supplier TIN was not provided!", 'r');
                        ...
                        if (isset($v_data['taxpayer'])){
                            $this->logger->write("Api : uploadimportedservice() : The supplier TIN was validated successfully!", 'r');
                            
                            $importedseller['ninbrn'] = $v_data['taxpayer']['ninBrn'];
                            $importedseller['legalname'] = $v_data['taxpayer']['legalName'];
                            $importedseller['businessname'] = $v_data['taxpayer']['businessName'];
                            $importedseller['mobilephone'] = $v_data['taxpayer']['contactNumber'];
                            $importedseller['emailaddress'] = $v_data['taxpayer']['contactEmail'];
                            $importedseller['address'] = $v_data['taxpayer']['address'];
                            
                        }
                    }
                    
                    //Let's populate the buyer details
                    ...
                    $buyer = array(
                        ...
                        'citizineship' => "",
                        'sector' => "",
                        'referenceno' => trim($json['VOUCHERNUMBER']),
                        'datasource' => $this->appsettings['WESERVICEDS']
                    );
                    ...
                    if (isset($json['INVENTORIES'])) {
                        foreach ($json['INVENTORIES'] as $obj){
                            $this->logger->write("Api : uploadimportedservice() : The PRODUCTCODE is " . $obj['PRODUCTCODE'], 'r');
                            ...
                            /**
                             * @desc The PO & Bill documents in some QBD versions do not provide for tax on purchases. Handle!
                             * @author francis.lubanga@gmail.com
                             * @date 2024-01-16
                             *
                             */
                            
                            if (trim($obj['TAXCODE']) == '' || empty($obj['TAXCODE'])) {
                                /**
                                 * Determine the tax code relevant to the product
                                 */
                                $taxid = $this->util->getinvoicetaxrate_v2($json['INDUSTRYCODE'], $json['BUYERTYPE'], trim($obj['PRODUCTCODE']), trim($companydetails->tin), $this->appsettings['OVERRIDE_TAXRATE_FLAG'], $this->appsettings['TAXPAYER_CHECK_FLAG']);
                                $this->logger->write("Api : uploadimportedservice() : The computed TAXID is " . $taxid, 'r');
                                $tr->getByID($taxid);
                                $taxcode = $tr->code;
                            } else {
                                /**
                            ...
                            if (trim($taxcode) == '' || empty($taxcode)) {
                                $this->logger->write("Api : uploadimportedservice() : The PRODUCTCODE " . $obj['PRODUCTCODE'] . " does not have a TAXCODE", 'r');
                                $this->message = "The PRODUCTCODE " . $obj['PRODUCTCODE'] . " does not have a TAXCODE!";
                                $this->code = '-999';
                                ...
                                if (trim($taxid) == '' || empty($taxid)) {
                                    $this->logger->write("Api : uploadimportedservice() : The TAXCODE on PRODUCTCODE " . $obj['PRODUCTCODE'] . " is not defined", 'r');
                                    $this->message = "The TAXCODE on PRODUCTCODE " . $obj['PRODUCTCODE'] . " is not defined!";
                                    $this->code = '-999';
                                    ...
                                    $body = '<html><body>
                                                <p>Hello,</p></br>
                                                <p>Your request to upload an imported services invoice failed with the following details;</p>
                                                <p>Number: <b>' . $vchnumber . '</b></p>
                                                <p>Error Code: <b>' . $this->code . '</b></p>
                                                <p>Error Message: <b>' . $this->message . '</b></p>
                                                </br>
                                    ...
                                    $this->logger->write("Api : uploadimportedservice() : The final TAXID is " . $taxid, 'r');
                                    ...
                                    if ($pricevatinclusive == 'NO') {
                                        $this->logger->write("Api : uploadimportedservice() : The price is tax exclusive!", 'r');
                                        ...
                                        //Manually calculate figures
                                        $this->logger->write("Api : uploadimportedservice() : Manually calculating tax", 'r');
                                        ...
                                        $this->logger->write("Api : uploadimportedservice() : The price is tax inclusive!", 'r');
                        ...
                        if (sizeof($goods) > 0) {
                            $this->logger->write("Api : uploadimportedservice() : The GOODS count: " . sizeof($goods), 'r');
                            $this->logger->write("Api : uploadimportedservice() : The TAX count: " . sizeof($taxes), 'r');
                            $this->logger->write("Api : uploadimportedservice() : The PAYMENTS count: " . sizeof($payments), 'r');
                            
                            $data = $this->util->uploadimportedservice($this->userid_u, $branch->uraid, $importedseller, $invoicedetails, $goods, $payments, $taxes);
                            $data = json_decode($data, true);
                            
                            if (isset($data['returnCode'])){
                                $this->logger->write("Api : uploadimportedservice() : The operation to upload the imported service invoice not successful. The error message is " . $data['returnMessage'], 'r');
                                //$this->util->createinappnotification(NULL, NULL, NULL, self::$module, self::$submodule, $operation, $event, $eventnotification, NULL, $this->f3->get('SESSION.id'), "The operation to upload the invoice by " . $this->f3->get('SESSION.username') . " was not successful");
                                $this->message = $data['returnMessage'];
                                $this->code = $data['returnCode'];
                                ...
                                if (trim($data['returnCode']) == '2253') {
                                    $this->logger->write("Api : uploadimportedservice() : The seller's reference is already in EFRIS. We are going to go ahead and query for the details in EFRIS.", 'r');
                                    
                                    $data_n = $this->util->invoiceinquiry($this->userid_u, NULL, NULL, NULL, NULL, $vchnumber, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
                                    $data_n = json_decode($data_n, true);
                                    
                                    if (isset($data_n['returnCode'])){
                                        $this->logger->write("Api : uploadimportedservice() : The operation to query the imported service invoice from EFRIS was not successful. The error message is " . $data_n['returnMessage'], 'r');
                                        $this->message = $data_n['returnMessage'];
                                        $this->code = $data_n['returnCode'];
                                    } else {
                                        $this->logger->write("Api : uploadimportedservice() : The operation to query the imported service invoice from EFRIS was successful.", 'r');
                                        /*
                                         {
                                         "page": {
                                         "pageNo": "1",
                                        ...
                                        if(isset($data_n['records'])){
                                            if ($data_n['records']) {
                                                foreach($data_n['records'] as $elem){
                                                    ...
                                                    if (isset($i_data['returnCode'])){
                                                        $this->logger->write("Api : uploadimportedservice() : The operation to download the imported service invoice details from EFRIS was not successful. The error message is " . $i_data['returnMessage'], 'r');
                                                        $this->message = $i_data['returnMessage'];
                                                        $this->code = $i_data['returnCode'];
                                                    } else {
                                                        $this->logger->write("Api : uploadimportedservice() : The operation to download the imported service invoice details from EFRIS was successful.", 'r');
                                                        ...
                                                        $inv_status = $this->util->createimportedservice($invoicedetails, $goods, $taxes, $buyer, $this->userid_u, $importedseller); //The $importedseller object holds the imported seller details at this stage
                                                        
                                                        if ($inv_status) {
                                                            $this->logger->write("Api : uploadimportedservice() : The imported service invoice was created on e-TW successfully", 'r');
                                                        } else {
                                                            $this->logger->write("Api : uploadimportedservice() : The imported service invoice was NOT created on e-TW", 'r');
                                                        }
                                                        
                                                        $this->message = 'The operation to upload the imported service invoice was successful';
                                                        $this->code = '000';
                                                    }
                                                }//foreach
                                            }
                                ...
                                $inv_status = $this->util->createimportedservice($invoicedetails, $goods, $taxes, $buyer, $this->userid_u, $importedseller);
                            ...
                            $this->logger->write("Api : uploadimportedservice() : No goods details were supplied!", 'r');
                            $this->message = "No goods details were supplied!";
                            $this->code = '-999';
                ...
                $this->logger->write("Api : uploadimportedservice() : The user is not allowed to perform this function", 'r');
                $this->message = "The user is not allowed to perform this function";
                $this->code = '0099';
            }
            
            
            $activity = 'uploadimportedservice: ' . $vchnumber . ': ' . $vchref . ': ' . $this->message;
            $windowsuser = trim($json['WINDOWSUSER']);
            $ipaddress = trim($json['IPADDRESS']);
            $macaddress = trim($json['MACADDRESS']);
            $systemname = trim($json['SYSTEMNAME']);
    ...
    function beforeroute(){
        $this->logger->write("Api : beforeroute() : ERP_USER_AUTH_SWITCH = " . trim($this->appsettings['ERP_USER_AUTH_SWITCH']), 'r');
        $this->logger->write("Api : beforeroute() : VERSION_MISMATCH_TREATMENT = " . trim($this->appsettings['VERSION_MISMATCH_TREATMENT']), 'r');
        ...
        if(sizeof($json) == 0 && empty($API_KEY)){
            ...
            if(empty($this->apikey)){
                ...
                if($apikey_check->dry ()){
                    ...
                    } elseif (trim($this->version) !== $this->appsettings['APPVERSION'] && trim($this->appsettings['VERSION_MISMATCH_TREATMENT']) == 'I'){
                        $this->logger->write("Api : beforeroute() : The versions do not match. Platform set to Ignore.", 'r');
                    } elseif (trim($this->version) !== $this->appsettings['APPVERSION'] && trim($this->appsettings['VERSION_MISMATCH_TREATMENT']) == 'B'){
                        $this->logger->write("Api : beforeroute() : The versions do not match. Platform set to block", 'r');
                        $this->code = '1003';
